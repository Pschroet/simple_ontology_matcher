...
def match_two_ontologies(results, onto, onto1):
    ...
    if onto is not None and onto1 is not None:
        connections = {"matches":[], "text": "Matching by searching for antonyms from one or more dictionaries (dictionary_antonym_matcher)"}
                                    ...
                                    for item1 in label1:
                                        for antonym in antonyms:
                                            if not already_matched and re.match(item1.get_text(), antonym, re.IGNORECASE):
                                                connections["matches"].append([i.name, item.get_text(), j.name, item1.get_text(), " are antonyms", "owl:inverseOf"])
