...
def match_two_ontologies(results, onto, onto1):
    ...
    if onto is not None and onto1 is not None:
        connections = {"matches":[], "text": "Matching by calculating the Levenshtein distances of the labels (1 < distance < 5)"}
                            ...
                            if label1 != []:
                                for item1 in label1:
                                    if len(item.get_text()) > 3 and len(item1.get_text()) > 3:
                                        ...
                                        #version 2
                                        if not already_matched and 1 < distance and distance < 5 and (distance < len(item.get_text())/3) and not re.match(i.name, j.name, re.IGNORECASE):
